...
// Secondary info: for former members this is the from-to period on the board,
// for current members this is their study status
...
$secondary_label = $is_former ? 'Zeit im Vorstand' : 'Studiert';

// only relevant for current board members
$board_since = $is_former ? '' : get_post_meta(get_the_ID(), $meta_prefix . '_board_since', true);
...
<article class="<?php echo esc_attr($card_classes); ?>">
    ...
    <div class="p-6 text-left flex flex-col items-start">
        ...
        <?php if ($secondary_info || $board_since): ?>
            <div class="mt-4 flex flex-col">
                <?php if ($board_since): ?>
                    <div class="flex-2">
                        <p class="text-xs text-text-light/70 dark:text-dark-text-muted/70 uppercase tracking-wide mb-1">Im Vorstand seit</p>
                        <p class="text-text-light dark:text-dark-text-muted text-sm flex items-center gap-2">
                            <?php echo esc_html($board_since); ?>
                        </p>
                    </div>
                <?php endif; ?>

                <?php if ($secondary_info): ?>
                    <div class="<?php echo $board_since ? 'pt-4' : ''; ?> flex-3">
                        <p class="text-xs text-text-light/70 dark:text-dark-text-muted/70 uppercase tracking-wide mb-1"><?php echo esc_html($secondary_label); ?></p>
                        <p class="text-text-light dark:text-dark-text-muted text-sm flex items-center gap-2">
